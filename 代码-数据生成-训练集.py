...
def countryflag(x):
    if x in 'Poland|Lithuania|Channel Islands|Lebanon|United Arab Emirates|Saudi Arabia|Unspecified|Brazil|European Community|Bahrai|RSA':
        y = 0
    else:
        y = 1
    return y
...
#取出某个月的订单数据，例如6月
mon=10 ######################修改月份
df_month = df_ord.query('order_month==@mon') # 选择所有月则，df_month = df_ord
#选出去重商品和顾客
...
#保存数据
df.to_csv('Merge_test/out_sample_0227_Oct.csv',encoding='utf-8_sig') ##输出样本
